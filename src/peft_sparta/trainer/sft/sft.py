...
class SFT:
    def __init__(self,
        ...
        if model_config['new_tokens'] and peft_config:
            ...
            if peft_config.pop('train_new_tokens'):
                ...
                new_token_ids = tokenizer.convert_tokens_to_ids(model_config['new_tokens'])
    ...
    def create_dataloader(self, dataset, batch_size, shuffle=False, max_token_len=None):
        ...
        if self.task == 'SEQ_CLS':
            ...
            # supported data formats for training (tokenization + labeling)

            def tokenize_text(example): # language modeling (no chat): used for continued pretraining or domain adaptation
            ...
            def tokenize_prompt_completion(example):  # no chat
            ...
            def tokenize_prompt_completion_chat(example):  # using chat template format: for instruction-following
